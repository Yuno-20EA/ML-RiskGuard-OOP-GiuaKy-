    ...
    if (!transA && !transB) {
        ...
        for (int i = 0; i < a_rows; ++i) {
            for (int k = 0; k < a_cols; ++k) {
                double temp = a_ptr[i * A.cols + k];
                for (int j = 0; j < b_cols; ++j) {
                    c_ptr[i * C.cols + j] += temp * b_ptr[k * B.cols + j];
                }
            }
        }
    } else if (transA && !transB) {
        // C = A^T * B
        for (int k = 0; k < a_rows; ++k) { // k is row of A^T, so col of A
            for (int i = 0; i < a_cols; ++i) { // i is col of A^T, so row of A
                double temp = a_ptr[i * A.cols + k]; // A(i, k)
                for (int j = 0; j < b_cols; ++j) {
                    c_ptr[k * C.cols + j] += temp * b_ptr[i * B.cols + j];
                }
            }
        }
    } else if (!transA && transB) {
        // C = A * B^T
        for (int i = 0; i < a_rows; ++i) {
            for (int j = 0; j < b_cols; ++j) { // j is col of B^T, so row of B
                double sum = 0.0;
                for (int k = 0; k < a_cols; ++k) { // k is col of A and col of B
                    sum += a_ptr[i * A.cols + k] * b_ptr[j * B.cols + k];
                }
        ...
        // C = A^T * B^T
        for (int i = 0; i < a_rows; ++i) { // i is col of A
            for (int j = 0; j < b_cols; ++j) { // j is row of B
                double sum = 0.0;
                for (int k = 0; k < a_cols; ++k) { // k is row of A and col of B
                    sum += a_ptr[k * A.cols + i] * b_ptr[j * B.cols + k];
                }
                c_ptr[i * C.cols + j] += sum;
            }
        }
